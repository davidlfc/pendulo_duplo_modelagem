...
theta_1_ini = %pi/2   //rad
theta_2_ini = %pi/2    //rad
...
function dy = penduloduplo(t,y)  //integração das equações diferenciais
    ...
    a22 = 1/3
    b1 = -y(3)*y(4)/2 * sin(y(1)-y(2)) + (lambda/2 + 1)*wp^2*lambda*sin(y(1))
...
scf(1);
plot2d(t,theta_1,2);
title("Theta 1 em função do tempo");
xlabel("t(s)");
ylabel("Posição angular(rad)");

scf(2);
plot2d(t,theta_2,3);
title("Theta 2 em função do tempo");
xlabel("t(s)");
ylabel("Posição angular(rad)");

scf(3);
plot2d(t,theta_1_p,4);
title("Theta 1 ponto em função do tempo");
xlabel("t(s)");
ylabel("Velocidade angular(rad/s)");

scf(4);
plot2d(t,theta_2_p,5);
title("Theta 2 ponto em função do tempo");
xlabel("t(s)");
ylabel("Velocidade angular(rad/s)");
...
title("diferença de theta 1 (nlin - lin)");
xlabel("t(s)");
ylabel("d theta1(rad)");
...
title("diferença de theta 2 (nlin - lin)");
xlabel("t(s)");
ylabel("d theta2(rad)");
...
title("diferença de theta 1 ponto (nlin - lin)");
xlabel("t(s)");
ylabel("d theta1 p(rad/s)");

//dif theta 2 ponto
scf(12);
plot2d(t,dif_theta_2_p,6);
title("diferença de theta 2 ponto (nlin - lin)");
xlabel("t(s)");
ylabel("d theta2 p(rad/s)");
